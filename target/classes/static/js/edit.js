...
window.onload = function () {
	...
	document.getElementById('confirmNext').onclick = function() {
	  ...
	  fd.append('tripId', document.getElementById('returnButton').dataset.tripId);	
	  ...
	  const obj = Object.fromEntries(fd)
	      fetch('http://localhost:8080/trip/edit', {
	      method: 'POST',
	...
	document.getElementById('returnButton').onclick = function() {
		const tripId = document.getElementById('returnButton').dataset.tripId;
		// 閲覧画面へ遷移
		const url = new URL('http://localhost:8080/trip/browseView');
